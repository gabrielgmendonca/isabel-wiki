...
1. Na página da obra/autor, inserir:
   ...
   ```
2. Criar/atualizar `wiki/divergencias/<slug>.md` com: posição de Kardec (citação), posição do complementar (citação), análise (divergência real / mudança de ênfase / aprofundamento / erro de interpretação) e status: `aberta` ou `conciliada`.
