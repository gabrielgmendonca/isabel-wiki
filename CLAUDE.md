...
- **Query**: usar qmd `query` para encontrar páginas relevantes (wiki/ primeiro, raw/ se necessário) → ler páginas encontradas → responder com citações começando por Kardec. Fallback: ler `wiki/sinteses/catalogo.md` se qmd indisponível (a home `index.md` é landing de trilhas, não listagem). **Default: oferecer arquivar** escolhendo o tipo adequado — `wiki/sinteses/` (panorama, conexões entre temas), `wiki/aprofundamentos/` (estudo de um tema/bloco doutrinário, ex.: subseção do LE, capítulo do ESE) ou `wiki/questoes/` (Q&A direta ancorada em uma única questão ou item pontual do Pentateuco — questão numerada de LE/LM/OQE ou item de C&I/ESE/Gênese). Só pular o oferecimento quando a pergunta for puramente operacional. Se arquivado, listar a nova página em `wiki/sinteses/catalogo.md` e registrar entrada em `log.md`.
