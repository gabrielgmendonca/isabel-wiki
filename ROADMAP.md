...
- [~] **Fragmentar Revista Espírita em `raw/`** — partição mensal feita (commits `35e04b6` + `bdeec27`): cada ano virou 12 arquivos `<ano>/<mes>-<nome>.md` (340-2141 linhas, mediana ~900). Já reduz substancialmente o overhead em `/ingest` e melhora score do `qmd` vs. ano monolítico. Per-artigo (150-300 linhas) ainda é meta original do roadmap; bloqueado por falta de marcadores claros de início de artigo dentro de cada mês — exigiria parser específico ou triagem manual. Avaliar se o ganho marginal compensa antes de prosseguir.
...
9. **Eficiência de tokens — eixo §9** — disciplina em queries `qmd` ✓ (2026-05-02), granularizar `convencoes-paginas.md` ✓ (2026-05-02), Revista Espírita parcial (partição mensal ✓; per-artigo pendente), Haiku para `/lint` e `/glossario` ✓ (2026-05-02), pré-resumo de obras monolíticas (LM/Gênese/ESE) ✓ (2026-05-02). Custo composto: ganho pequeno por turno, alto somado em meses de uso. Resta avaliar se o per-artigo da Revista Espírita compensa o custo de implementação — partição mensal já cobre boa parte do overhead, então provavelmente fica como baixa prioridade.
