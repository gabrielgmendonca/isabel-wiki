...
- [ ] **Linkar referências bíblicas** — `(Mateus 5:3)`, `(João 14:2)` etc. NT linka interno (`wiki/biblia/...`); AT linka externo (bibliaonline.com.br). Pré-requisitos abaixo (publicação do NT, mapping bíblico em `link_citations.py`).
...
- [ ] **Bible mapping em `link_citations.py`** — destravado pela decisão. `(Mateus 5:3)` → `[[wiki/biblia/mateus/5#v3|Mateus 5:3]]` para NT; `(Gênesis 1:1)` → URL externa para AT. Lista canônica de 66 livros + variantes (Mt/Mateus, Gn/Gênesis, 1Co/1 Coríntios). Cobre todas as ocorrências `(<livro> <cap>:<vers>)` em `wiki/**`.
...
- [ ] **Hook PostToolUse para mirror local `wiki/**` → `/tmp/quartz/content/wiki/**`** — caso concreto (2026-05-22): editei `wiki/aprofundamentos/silencio-interior-o-ser-consciente.md` várias vezes, mas o `npx quartz build --serve` observa o mirror em `/tmp/quartz/content/`, não o repo. Mudanças não aparecem no preview até refazer `cp` manual ou re-rodar `./scripts/serve-local.sh` (rebuild completo ~10-15s vs. hot-rebuild incremental do Quartz em ~6s). Hook `.claude/hooks/mirror-to-quartz.py` em `Edit|Write|MultiEdit` para `wiki/**/*.md` (e `index.md` da raiz) copia o arquivo editado para o mirror; Quartz hot-rebuilda. Trava de segurança: só atua se `/tmp/quartz/content/wiki/` existir (i.e., dev já rodou `serve-local.sh` antes — caso contrário noop silencioso, sem impacto em sessões que não usam preview). Limitação consciente: pre-processadores (`link_citations.py`, `wrap_glossary_terms.py`, `inject_copyright.py`) **não** rodam no arquivo espelhado — preview de dev fica "raw" (sem auto-link de citação Kardecpedia, sem `<abbr>` de glossário cultural, sem callout de direitos). Aceitável para inspeção visual de estrutura, tipografia, Mermaid e wikilinks internos; CI continua aplicando todos os transforms no build de deploy, então diff entre preview local e deploy é conhecido e contido. Custo de implementação: ~30 linhas Python análogas ao `lint-on-edit.py`.
...
- **Publicar NT em `wiki/biblia/` (§4)** — destravado pela decisão (c) híbrido de 2026-05-22. Próximo passo: definir granularidade (arquivo-por-capítulo vs arquivo-por-livro) e schema de frontmatter. ~260 arquivos ou ~27 dependendo da escolha.
- **Bible mapping em `link_citations.py` (§4)** — destravado pela mesma decisão. Função pura, escopo claro, suite de testes paralela à do mapping Kardec. Pode rodar antes ou depois da publicação (NT ainda não publicado = link interno quebrado, mas válido como ponteiro futuro).
