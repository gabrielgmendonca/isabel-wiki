...
9. **Eficiência de tokens — eixo §9** — disciplina em queries `qmd` ✓ (2026-05-02), granularizar `convencoes-paginas.md` ✓ (2026-05-02), fragmentar Revista Espírita por artigo, Haiku para tarefas determinísticas. Custo composto: ganho pequeno por turno, alto somado em meses de uso. Próximo passo natural: fragmentar Revista Espírita por artigo (cada ano hoje é 1 arquivo de 8-13K linhas; reestruturação melhora `qmd` e reduz contexto em `/ingest`).
