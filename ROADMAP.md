...
- [ ] **Ver o tema sem gastar uma palestra — `slides/themes/preview.md` + lint de overflow** — hoje o único jeito de inspecionar o `isabel.css` (125 linhas) é buildar um deck inteiro: as descobertas de legibilidade e de capacidade do slide registradas acima custaram uma palestra real (`slides/indulgencia/`) e só apareceram ao renderizar o PDF. Dois passos, ambos **camada 0**:
  - **(a) Deck de demonstração** `slides/themes/preview.md` — um slide por classe (capa, `.pergunta`, `.quote`, `.section`, split background `![bg right:43%]`, síntese de 5 bullets no limite conhecido), buildado com o `build_deck.py` que já existe. Vira teste de regressão visual sempre que se mexer no tema, offline e versionado. ~30 linhas, zero dependência nova.
  - **(b) `check_slide_overflow`** (severidade `info`) — estima overflow por contagem de linhas × classe, calibrado no caso conhecido (~5 bullets longos a 32px com padding 80/100 colidem com o rodapé). Roda sobre `slides/**/deck.md`.
