...
9. **Eficiência de tokens — eixo §9** — disciplina em queries `qmd` ✓ (2026-05-02), granularizar `convencoes-paginas.md` ✓ (2026-05-02), Revista Espírita parcial (partição mensal ✓; per-artigo pendente), Haiku para `/lint` e `/glossario` ✓ (2026-05-02). Custo composto: ganho pequeno por turno, alto somado em meses de uso. Próximo passo natural: pré-resumo de obras monolíticas (LM ~12K linhas) com `_index.md` + `_resumo.md` gerados por Haiku — ou avaliar se a partição mensal da Revista já cobre o suficiente e o per-artigo fica como item de baixa prioridade.
