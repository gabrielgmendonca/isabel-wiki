...
- [ ] **Testes do `scripts/link_citations.py`** — pipeline crítico do build, sem testes hoje. Casos-canto a cobrir: citação dentro de code block, em heading, em link já existente, sigla ambígua (LE vs Léon Denis abreviado?), citação composta (`LE q. 150-152`), citação dentro de fronteira de wikilink. Adicionar fixture e rodar em CI antes que regressão silenciosa apareça em produção.
