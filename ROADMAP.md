...
- [ ] **Check determinístico de resolução de citação** (`check_citation_resolves`) — fecha o ponto cego de fidelidade do `/ingest`: o gate humano aprova a *intenção* (plano), não a fidelidade da citação. Solução determinística, não LLM (decisão 2026-05-19): novo check em `lint_wiki.py` que extrai `(sigla, ref)` das páginas e bate contra o locus real em `raw/` — reusa o parser e os 25 testes de `link_citations.py`. Camada baixa: prova que o locus **existe**; "o trecho sustenta a afirmação?" continua editorial. **Dependência:** granularidade por questão/item em `raw/kardec/pentateuco/` ainda é item aberto (§4). Enquanto não existe, `/ingest` fica na variante leve (Passo 7 lista citações novas para spot-check).
