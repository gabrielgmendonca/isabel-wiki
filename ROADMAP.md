...
- [ ] **Fase 3 — cobertura.** Estender a aspas inline (hoje só double-quote adjacente à citação) e a aspas de loci que o `cite.py` não fragmenta (Introdução/Conclusão por item); onde não resolve, abster.
  - **Gap descoberto (2026-06-17): blockquotes não são varridos.** `check_literal_quote_exists`/`_scan_literal_quotes` fazem `_strip_blockquotes` antes de casar — então TODA aspa de Kardec em blockquote (`> "..." (cite)`) nunca foi checada; as 128 tratadas eram só as inline. Achado concreto: `wiki/divergencias/escravidao-em-efesios-6.md` tinha 2 blockquotes fabricados atribuídos a q.825/q.829 (corrigidos à mão para q.829/q.830). Estender o scan a blockquotes e rodar o mesmo ciclo (dry-run + adversário) é o próximo passo.
...
- **Triagem das 128 aspas restantes do §12** — as 25 mal-atribuídas (classe 2) **já corrigidas** (2026-06-15) via índice reverso; restam 97 fabricadas + 8 paráfrases + 23 incertas (`reports/citacao/triagem-aspas.md`), que exigem de-quote/reancoragem manual. **Dry-run de veredictos rodado (2026-06-17, `reports/citacao/triagem-veredictos-*.md`)** — falta aplicar em lotes. Cruza com a triagem cit do §11.
