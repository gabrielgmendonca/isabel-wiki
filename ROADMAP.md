...
- [ ] **Pre-flight check em `/ingest` Passo 0** — estender Passo 0 da skill com checagem de (a) existência de `raw/<caminho>` na worktree atual e (b) `git status` worktree vs `main` antes de iniciar análise. Hoje o Passo 0 só valida escopo doutrinário, não pré-requisitos materiais; o report de uso (2026-05-04) registrou ao menos dois casos de `/ingest` desperdiçado por raw inexistente na worktree (Divaldo Franco) ou worktree atrás de main mascarando arquivos. Custo: ~30s no início; benefício: evita ciclo completo de análise descartado.
...
- [ ] **Enriquecer `wiki/aprofundamentos/sexualidade-em-andre-luiz.md`** com material de *Sexo e Destino* (1963, ingerido em 2026-05-03) — em particular a passagem de Félix sobre dignidade dos homossexuais (Parte 2, cap. 5), o conceito de [[wiki/conceitos/possessao-partilhada|possessão partilhada]] (Parte 1, cap. 8) e a tese da sexualidade como construtora de destino. Próximo ciclo Query.
...
- [ ] **Skill `/ship` para o ciclo commit→land** — `scripts/land-to-main.sh` já existe e funciona, mas cada uso exige sequência manual (stage → commit → invocar script → tratar conflicts). Skill `/ship` empacotaria: (1) revisa diff e propõe mensagem de commit coerente, (2) invoca `land-to-main.sh`, (3) em conflict de página com `merge=union` deixa rerere agir, em página de personalidade aplica protocolo de união cronológica, (4) roda `scripts/lint_wiki.py` (validação determinística) ao final — **não** invoca a skill `/lint` (que puxa LLM). Reduz fricção em ~15 commits/semana medidos no report de uso (2026-05-04). Depende de "Rule `convencoes-merge.md`" abaixo estar escrita.
- [ ] **Hook `PostToolUse` para lint leve por arquivo** — hook em `Edit|Write|MultiEdit` para `wiki/**/*.md` que invoque `scripts/lint_wiki.py --file <path>` (modo single-file a ser adicionado: parse de frontmatter + sanidade de wikilinks da página, sub-segundo). Não substitui `/lint` global; pega frontmatter inválido (data fora de ISO 8601, tag fora dos namespaces canônicos) e wikilink quebrado **no momento da edição**, em vez de no fim do `/ingest`. Pré-requisito: `lint_wiki.py` ganhar flag `--file` que rode só os checks isoláveis. Cuidado para o hook não tornar Edits perceptivelmente lentos.
- [ ] **Rule `convencoes-merge.md`** — criar rule (paths: `wiki/personalidades/**`, `log.md`, `catalogo.md`) documentando: (a) `.gitattributes` com `merge=union` em log/catalogo/ROADMAP, `merge=ours` em estatisticas-da-wiki, (b) padrão "união cronológica" para conflicts em páginas de personalidade (que não estão no `merge=union`), (c) sempre rodar `scripts/lint_wiki.py` antes de `git rebase --continue`. O comportamento já existe na prática (5+ sessões resolvendo na mão); a rule só codifica para evitar re-derivação. Pré-requisito da skill `/ship` acima.
