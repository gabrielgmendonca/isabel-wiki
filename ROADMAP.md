...
- [ ] **`link_citations.py` não deve injetar wikilink em heading** — caso concreto (2026-05-22) em `wiki/conceitos/bonus-hora.md`: o H3 `### O sistema (segundo a senhora Laura, *Nosso Lar*, cap. 22)` vira `### O sistema [[wiki/obras/nosso-lar|(segundo a senhora Laura, *Nosso Lar*, cap. 22)]]`. Funciona como Markdown, mas polui a anchor do Quartz (slug do heading vira algo como `o-sistema-segundo-a-senhora-laura-nosso-lar-cap-22`) e cria link "decorativo" onde a intenção era título de seção. Adicionar safe zone em `SAFE_RE` para linhas que começam com `^#{1,6} ` (preservar heading inteiro). Avaliar também: a citação em heading deveria ser **removida** do título e migrar para a primeira linha do corpo da seção?
