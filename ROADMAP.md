...
- **Backup e portabilidade (§5)** (2026-05-16) — README §Setup/Migração + `.env.example` + `scripts/{preflight,bootstrap}.sh`; bump Python 3.12→3.14; 10 memórias locais consolidadas em docs rastreados + podadas. Checklist executável para migrar de máquina.
...
4. ~~**Backup do que está fora do git (§5)**~~ — concluído em 2026-05-16 (README §Setup/Migração + `.env.example` + `scripts/{preflight,bootstrap}.sh` + bump Python 3.14 + consolidação das memórias; ver §5 e bloco "Concluído").
